...
const repositoryRoot = resolve(__dirname, '../../repositories/parcel2');
...
const pageAddress = `http://localhost:${Ports.Parcel2}`;
...
test.beforeAll(async () => {
  ...
  // wait until the "Built in ..." message
  await handle.waitForTextInStdout('Built');
});
